...
export async function deleteMedia(id:string){await requireAdmin();const media=await db.mediaAsset.findUniqueOrThrow({where:{id}});if(await db.recipe.count({where:{image:media.url}}))return;await db.mediaAsset.delete({where:{id}});try{await unlink(path.join(process.cwd(),"public",media.url))}catch{}revalidatePath(studioPath("/medias"));}
...
export async function importRecipes(formData:FormData){await requireAdmin();const file=formData.get("file");if(!(file instanceof File)||!file.size||file.size>5*1024*1024)return;const text=await file.text();let rows:unknown[];if(file.name.toLowerCase().endsWith(".json")){const parsed=JSON.parse(text);rows=Array.isArray(parsed)?parsed:(parsed as {recipes?:unknown[]}).recipes??[];}else rows=parseCsv(text).map(record=>Object.fromEntries(Object.entries(record).map(([key,value])=>[["ingredients","steps","dietary","tags","collections"].includes(key)?key:key,["ingredients","steps","dietary","tags","collections"].includes(key)?value.split("|").filter(Boolean):value])));for(const raw of rows.slice(0,1000)){const record=raw as Record<string,unknown>;const candidate=recipeInputSchema.safeParse({...record,image:String(record.image||"/hero-bowl.png"),status:"DRAFT",featured:false,scheduledFor:null});if(!candidate.success)continue;const data=candidate.data;let slug=slugify(data.title);if(await db.recipe.findUnique({where:{slug}}))slug=`${slug}-${crypto.randomUUID().slice(0,5)}`;await db.recipe.create({data:{...data,slug,publishedAt:null,ingredients:{create:data.ingredients.map((label,position)=>({...parseIngredient(label),position}))},steps:{create:data.steps.map((body,position)=>({body,position}))}}});}revalidatePath(studioPath());redirect(`${studioPath()}?imported=1`);}
